...
%
As part of the course creation process, we will determine if your
course is private (to your class only) or is a community course
(accessible to anybody with a Diderot account).
%
For any course, we can set a passkey that can be used for students to
self-enroll to the course.
%


\begin{gram}[Public and Private Materials]

A typical course on Diderot consists of one or more online books.
%
Each book in turn can be made completely private or publicly readable.
%
A private book is only accessible to enrolled students of the course.
%
A public book is accessible to everybody, including users not logged-on to Diderot.
%

A typical Diderot course consists of a textbook or lecture notes,
which is sometimes public, and a number of private book such as
assignments, quizzes, and exams, which are only accessible to the
enrolled students.
\end{gram}
...
%
When a student logs onto Diderot, they can register for a private
course with the correct passkey.
%
...
\includegraphics[width=0.9\textwidth]{lms/media/upload-slides.jpg}
\end{gram}
...
%
To register, the students will need to use an email address from an instructor-speficied  \defn{domain}.
%
