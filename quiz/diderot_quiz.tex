...
The exams also support \href{sec:quiz::algorithmic-feedback}{algorithmic feedback}, where the
...
\label{sec:quiz::algorithmic-feedback}

\begin{cluster}
\label{grp:grm:quiz::supports}

\begin{gram}
\label{grm:quiz::supports}
\label{sec:quiz::algorithmic-feedback}
Diderot supports algorithm feedback and grading by allowing
instructors to use programs written in various languages (e.g.,
Javascript, Python, and really anything) to analyze student answers
and give them feedback automatically.
This comes in three flavors:
\begin{itemize}
\item Instant algorithmic feedback to students answers by using Javascript and Python
\item Instant autograding and feedback  by using Javascript and Python
\item Full autograding and feedback by using Docker containers (using essentially any language and system).
\end{itemize}
...
\begin{cluster}
\label{grp:grm:quiz::instant-algorithmic-feedback}

\begin{gram}[Instant Algorithmic Feedback]
\label{grm:quiz::instant-algorithmic-feedback}
This allows the students to request feedback every time they fill a free-response or fill-in-blanks box.
To implement this the instructor provides a Javascript or Python code than can digest the students answer and give them feedback.
There is no limit to what this code can do: it can run a simple analysis,
perform natural language comprehension, or can even run a formal analysis tool (relevant if the answer has a formal structure such as a mathematical formalism).
This instant feedback is not meant for grading per se, but for actually helping students.
The feedback scripts runs on the cloud and consumes no resources of
the user's device; response time for this service is typically a
couple of seconds.
...
\begin{cluster}
\label{grp:grm:quiz::instant-algorithmic-grading}

\begin{gram}[Instant Algorithmic Grading]
\label{grm:quiz::instant-algorithmic-grading}
This allows the students to request a grade every time they complete a
test problem (interactive mode) or a test (batch mode).
This is automatically handled for multiple choice questions.
For free response or fill-in-the-blanks questions, the instructor
provides a Javascript or Python code that can digest the students
answer and assign them a grade.
When doing the grading, the code has access to all parts of the problem and the students' answer to them, so the grades could be adjusted based on context.
There is no limit to what the grader code can do.
The grader runs on the cloud and consumes no resources of
the user's device; response time for this service is typically a
couple of seconds.
...
\begin{cluster}
\label{grp:grm:quiz::full-algorithmic-grading}

\begin{gram}[Full Algorithmic Grading]
\label{grm:quiz::full-algorithmic-grading}
This allows the students to request a grade every time they complete a test.
To enable this, the instructor provides a Docker container that it set
up to digest a test submission and assign it a grade and generate feedback.
When doing the grading, the code has access to whole submission.
There is no limit to what the grader code can do.
The grader runs on the cloud and consumes no resources of
the user's device; response time for this service is typically a
couple of minutes.
