...
the structure of the LaTeX sources described here.
%
...
%
In diderot, each and every piece of text must be inside of an atom and just like any other LaTeX section, an atom cannot be included inside of a LaTeX environment.
%
...
In addition to the atoms above, Diderot also supports figure and table atoms, which have a slightly different syntax
\begin{lstlisting}
\begin{<atom>}[Optional title]
<atom body>
\caption{Caption}
...
The reason for this exception is consistency with LaTeX.
\end{important}

\begin{important}[Nesting of Atoms]
Atoms cannot be nested inside each other and they cannot be nested inside Latex environments. This is similar to not being able to define a LaTeX section inside a LaTeX environment.
...
But is in wrong because it includes an atom (a kind of section) in a LaTeX environment.
\end{gram}
\end{itemize}
\end{lstlisting}
...
You can embed a video to your notes by using \lstinline`\video{url}{text}` command.
%
...
\subsection{Structuring your Sources and Compilation}
\label{sec:dc::latex-structure}
You can translate LaTeX to PDF, HTML, and XML by using the \lstinline`dc` binary.  
%
For this to work smoothly, we strongly advise following the structure
...
Math packages, will be ignored for XML generation.  Do not include any
command definitions in this file.
...
To simplify publishing on Diderot, we recommend organizing your book
sources as outlined below. 

\begin{gram}[Booklets]
 
 Booklets are books that don't have parts. We recommend creating one directory per chapter and placing a single main.tex file to include all contain that you want.  Place all media (images, videos etc) under a media/ subdirectory. 
\begin{itemize}  
