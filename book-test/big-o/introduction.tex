    ...
    \begin{algorithmic}[1]
        \Procedure{LinearSearch}{list $A$, size of list $n$, value $v$}
            \For{$i = 0$ to $n-1$} \Comment{This function performs everything in the loop $n$ times}
                \If{$A[i] == v$}   \Comment{This is a constant time operation}
                \State \Return $i$  \Comment{This is a constant time operation}
                \EndIf
            \EndFor
            \Return False
        \EndProcedure
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        \Procedure{BinarySearch}{sorted list $A$, size of list $n$, value $v$}
            \State $left = 0$
            \State $right = n$
            \While{$left < right$}
            \State $middle = (left + right) // 2$ \Comment{The function eliminates half of $A$ at each step}
            \If{$A[middle] \leq v$} \Comment{This is a constant time operation}
            \State $left = middle + 1$ \Comment{This is a constant time operation}
            \Else 
            \State $right = middle$ \Comment{This is a constant time operation}
            \EndIf
            \EndWhile
            \If{$left > 0 \textbf{ and } A[left - 1] == v$} \Comment{This is a constant time operation}
            \State \Return $left$ \Comment{This is a constant time operation}
            \Else 
            \State \Return False \Comment{This is a constant time operation}
            \EndIf
        \EndProcedure
    \end{algorithmic}
\end{algorithm}
