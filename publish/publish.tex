...
\begin{gram}[Using Diderot Latex Files for Live Updates]
\label{grm:publisg::diderot-atomic}
When you use the Diderot compiler, \lstinline`dc`, in addition to an XML file, it will also generate a \defn{Diderot LateX} file.  For example, if you run 
...
This file is ``fully atomized'' or \defn{atomic} in the sense that the contents has been partitioned into  blocks, representative units of Diderot.
%
...
\begin{note}[Atomic Sources]
%
The atomic file that the Diderot compiler generates is a special instance of a a ``fully labeled source file''.
%
...
%
In the unlikely even that you have not stored that file, you may find a copy of it in the temporary directory supplied to the compiled.  The file will look like this \lstinline`Diderot_main_UUID.tex` where the string \lstinline`UUID` is replaced by a UUID string and \lstinline`main` is the name of the original file.
%
