...
Putting aside the high-level questions and assuming that we allow
...
perhaps the easiest way is to edit an affected atom on Diderot.  This is often sufficient to fix small typos.  (Don't forget to update your LaTeX or Markdown source).

\item \textbf{XML edits:}
%
For larger changes, you can edit the XML of the chapter directly and reupload the XML.  To edit the XML, it is very important no to alter \lstinline`label` fields.  The contents of all else can be edited.
...
For LaTeX, there is a way to write your sources in such that essentially any updates except deletion of released sections of atoms are possible after release.  The idea is to completely atomize your chapter by making sure that each piece of text is inside of an atom and giving each section and each atom a unique label.
%
