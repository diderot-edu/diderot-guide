...
Diderot is somewhat conservative about the kinds of updates it support to existing content, because it does not want to allow student author content to be deleted by instructor actions.  
%
Diderot assumes that a chapter will not undergo major updates after it is released, but smaller updates, e.g., typo fixes, addition of some text, etc could be common. 
\end{gram}


\begin{gram}[Technical Issues with Live Updates]
One issue with live-updates is identifying what has been changed
and what has not.
%
Let's say that you have published and released a chapter on Diderot and later realized that you want to make some updates, and so you edited the chapter sources.
%
