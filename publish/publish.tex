...
The simplest answer to the problem above is to simply publish a new versions (revisions) instead of modifying existing versions.
%
...
%
After releasing the chapter, you want to make some significant changes, one way to do this is to simply create a chapter $2.1$ and release this as the version 1 of the homework.
%
Further versions can be released as chapters $2.2$, $2.3$, etc.
\end{gram}

\begin{gram}[Other Approaches]
If you don't want to use versioning, but update the content live and ``in place'' then there are other possible solutions.  Please read on.
\end{gram}
...
For the aforementioned reasons, Diderot is somewhat conservative about the kinds of updates it support to existing content.
